...
import { 
    ChevronLeft, ChevronRight, Sparkles, Image as ImageIcon, RefreshCw, Save, UserX,
    ZoomIn, ZoomOut, RotateCcw, X, Maximize2
} from 'lucide-react';
...
export default function StylistColumnCalendar({
    ...
    // Reference Photo Lightbox & Interactive Zoom State
    ...
    // Lock body scroll whenever modal or photo preview is active
    useEffect(() => {
        if (selectedApt || activePhotoUrl) {
    ...
    }, [selectedApt, activePhotoUrl]);

    // Global keyboard shortcuts (ESC, +, -, 0)
    useEffect(() => {
        if (!activePhotoUrl && !selectedApt) return;
    ...
    return (
        <div className="flex flex-col h-full bg-slate-950/80 rounded-3xl border border-white/10 overflow-hidden shadow-2xl backdrop-blur-xl">
            ...
            {/* ── High-Res Interactive Image Zoom Viewer (Rendered via Portal) ── */}
            {activePhotoUrl && createPortal(
                <div
                    ...
                    >
                        <div className="flex items-center gap-2 sm:gap-3">
                            <span className="text-xs font-black text-amber-400 bg-amber-500/10 border border-amber-500/20 px-3 py-1.5 rounded-xl flex items-center gap-1.5">
                                <ImageIcon size={14} /> Foto de Referencia
                            </span>
