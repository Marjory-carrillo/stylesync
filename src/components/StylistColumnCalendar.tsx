...
import { ChevronLeft, ChevronRight, Sparkles, Image as ImageIcon, RefreshCw, Save, UserX } from 'lucide-react';
...
export default function StylistColumnCalendar({
    ...
    const getAppointmentTotalPrice = (apt: Appointment) => {
        ...
        // 2. Base service price + extra options (+$X MXN) + catalog add-on services
        ...
        <div className="flex flex-col h-full bg-slate-950/80 rounded-3xl border border-white/10 overflow-hidden shadow-2xl backdrop-blur-xl">
            ...
            {selectedApt && (
                <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 animate-fade-in">
                    <div className="bg-slate-900 border border-white/15 rounded-3xl p-6 max-w-md w-full space-y-4 shadow-2xl relative text-left">
            ...
            {/* ── High-Res Image Modal Preview Overlay ── */}
            {activePhotoUrl && (
                <div
                    className="fixed inset-0 z-50 bg-black/90 backdrop-blur-xl flex items-center justify-center p-4 animate-fade-in cursor-zoom-out"
                    onClick={() => setActivePhotoUrl(null)}
                >
                    <div className="relative max-w-3xl max-h-[90vh] w-full flex flex-col items-center justify-center">
                        <button
                            onClick={() => setActivePhotoUrl(null)}
                            className="absolute -top-12 right-0 p-2 rounded-full bg-white/20 hover:bg-white/30 text-white font-black transition-colors cursor-pointer"
                        >
                            ✕ Cerrar
                        </button>
                        <img
