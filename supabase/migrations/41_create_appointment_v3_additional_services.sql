-- Migration 41: Drop existing overloads and recreate create_appointment_v3 with additional_services, booking_source, and commission
...
DROP FUNCTION IF EXISTS create_appointment_v3(uuid, text, text, bigint, bigint, date, time, text[], text, numeric);
...
BEGIN
    ...
    INSERT INTO appointments (
        ...
        stylist_id, date, time, status, additional_services,
        booking_source, marketplace_commission_amount
    ) VALUES (
        p_tenant_id, p_client_name, p_client_phone, p_service_id,
        p_stylist_id, p_date, p_time, 'confirmada', p_additional_services,
        p_booking_source, p_commission_amount
    )
